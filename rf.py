...
def main():
    ...
    # ========================= Step 6.5: Core set stratified =========================

    from sklearn.model_selection import StratifiedShuffleSplit

    desired_core_set_size = 5000  # Adjust based on desired size
    sss = StratifiedShuffleSplit(n_splits=1, train_size=desired_core_set_size, random_state=42)
    for core_indices, _ in sss.split(X_core, y_core):
        X_core2 = X_train_full_var[core_indices]
        y_core2 = y_train_full.iloc[core_indices].reset_index(drop=True)

    rf_core2 = RandomForestClassifier(n_estimators=200, n_jobs=-1, class_weight='balanced_subsample', max_depth=20, bootstrap=True)
    rf_core2.fit(X_core2, y_core2)  # Corrected line
    y_pred_core2 = rf_core2.predict(X_test_full_var)
    print("\nClassification Report (Core2 Set Model on Test Data):")
    print(classification_report(y_test_full, y_pred_core2, target_names=class_names, zero_division=0))
