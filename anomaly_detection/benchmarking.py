...
class Benchmarking:
    ...
    def visualize_feature_distributions(self):
        ...
        for i in range(num_features):
            ...
            sns.kdeplot(self.X_full[:, i], label='Full Dataset', shade=True)
            sns.kdeplot(self.X_core[:, i], label='Core Dataset', shade=True)
            plt.title(f'Distribution of Feature: {self.feature_names[i]}')
            plt.legend()
            plt.show()
