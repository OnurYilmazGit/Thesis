...
def main():
    ...
    # ... [Previous Steps 1-4 remain unchanged] ...

    # ================================
    # Step 5: Creating Class-Balanced Core Set
    # ================================
    print("\n=== Step 5: Creating Class-Balanced Core Set ===")
    start_time = time.time()

    # Determine the minimum class count to balance all classes
    min_class_count = y_train_full.value_counts().min()
    print(f"Minimum class count for balancing: {min_class_count}")

    # Perform random under-sampling for each class
    balanced_indices = y_train_full.groupby(y_train_full).apply(
        lambda x: x.sample(min_class_count, random_state=42)
    ).index

    # Since y_train_full is a pandas Series and X_train_full_var is a NumPy array,
    # we need to get integer indices
    balanced_indices = balanced_indices.get_level_values(1).tolist()

    # Select the samples from X_train_full_var using balanced_indices
    X_core_balanced = X_train_full_var[balanced_indices]
    y_core_balanced = y_train_full.iloc[balanced_indices].reset_index(drop=True)

    # Verify the class distribution in the balanced core set
    class_distribution_balanced = y_core_balanced.value_counts(normalize=True).sort_index()
    print("\nClass distribution in balanced core set (proportion):")
    print(class_distribution_balanced)

    # Record execution time
    end_time = time.time()
    execution_times['Creating Class-Balanced Core Set'] = end_time - start_time
    print(f"Class-balanced core set created in {execution_times['Creating Class-Balanced Core Set']:.2f} seconds.")

    # ================================
    # Step 6: Training Random Forest on Balanced Core Set
    # ================================
    print("\n=== Step 6: Training Random Forest on Balanced Core Set ===")
    start_time = time.time()

    # Initialize the Random Forest classifier
    rf_core_balanced = RandomForestClassifier(
        n_estimators=150,
        n_jobs=-1,
    ...
    # Train the classifier on the balanced core set
    print("Training Random Forest on balanced core set...")
    rf_core_balanced.fit(X_core_balanced, y_core_balanced)
    print("Training completed.")

    # Evaluate the model on the test data
    y_pred_core_balanced = rf_core_balanced.predict(X_test_full_var)

    # Adjust target names based on actual labels present in y_test_full
    labels = np.unique(y_test_full)
    target_names_adjusted = [class_names[int(i)] for i in labels]

    print("\nClassification Report (Balanced Core Set Model on Test Data):")
    print(classification_report(
        y_test_full,
        y_pred_core_balanced,
        labels=labels,
        target_names=target_names_adjusted,
        zero_division=0
    ))

    # Record execution time
    end_time = time.time()
    execution_times['Training on Balanced Core Set'] = end_time - start_time
    print(f"Random Forest trained and evaluated on balanced core set in {execution_times['Training on Balanced Core Set']:.2f} seconds.")
