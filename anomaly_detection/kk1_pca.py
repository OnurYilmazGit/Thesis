...
def main():
    ...
    try:
        ...
        X_full_pca = pca_vis.fit_transform(X_train_full_var)
        X_compressed_pca = pca_vis.transform(X_core_pca)

        plt.figure(figsize=(10, 8))
        plt.scatter(X_full_pca[:, 0], X_full_pca[:, 1], c=y_train_full, cmap='viridis', alpha=0.1, label='Full Data')
        plt.scatter(X_compressed_pca[:, 0], X_compressed_pca[:, 1], c=y_core, cmap='viridis', edgecolor='k', label='PCA Compressed Data')
        ...
        benchmark.compare_model_performance(rf_full, rf_core, rf_core_pca)
