...
def main():
    ...
    X_train_full_scaled_fs, selected_features = lasso_feature_selection(X_train_full_scaled, y_train_full, alpha=0.005)
    ...
    rf_full = RandomForestClassifier(max_depth=200, n_jobs=-1, random_state=42)
    ...
    compression_ratio = 0.003  # Adjust as needed
    ...
    def visualize_tsne_opentsne(X, y, n_components=2, perplexity=30, n_iter=500):
        """
        Visualizes the dataset using openTSNE on CPU.
        
        Args:
            X: The feature matrix.
            y: The target labels.
            n_components: Number of dimensions for t-SNE (default is 2 for 2D visualization).
            perplexity: Controls the number of neighbors used in t-SNE.
            n_iter: Number of iterations for optimization.
        """
        print("\n=== Visualizing t-SNE with openTSNE ===")
        ...
        plt.title("t-SNE Visualization (openTSNE)")
        plt.xlabel("t-SNE Component 1")
        plt.ylabel("t-SNE Component 2")
        plt.legend(title="Classes", bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.tight_layout()
        plt.savefig("tsne_opentsne_visualization.png")
        plt.show()
