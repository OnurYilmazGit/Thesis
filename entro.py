...
def main():
    ...
    # Create core set from from the half of the high-entropy samples
    ...
    # Evaluate the core model on the full test data
    y_pred_core = rf_core.predict(X_test_full_scaled)
    print("\nClassification Report (Core Set Model on Full Test Data):")
    ...
    print("\nClassification Report (Full Model on Full Test Data):")
    ...
    metrics_selected = evaluate_model(rf_full, X_test_full_scaled, y_test_full, class_names, "Full Model", "full_model")
    print("Accuracy of Full Model on Full Test Data:", metrics_selected['accuracy'])

    # Core model evaluation
    metrics_core = evaluate_model(rf_core, X_test_full_scaled, y_test_full, class_names, "Core Set Model", "core_model")
    print("Accuracy of Core Model on Full Test Data:", metrics_core['accuracy'])



   # =============================================
    # Step 7: Evaluating Core Set Model on Full Test Data
    # =============================================
    print("\n=== Step 7: Evaluating Core Set Model on Full Test Data ===")

    # Core model on full test data
    metrics_full_core = evaluate_model(rf_core, X_test_full_scaled, y_test_full, class_names,
                                       model_name="Core Set Model on Full Test Data", filename_prefix="core_model_on_full_test")
    ...
    summary_df = pd.DataFrame({
        ...
        'Samples': [X_train_full_scaled.shape[0], X_core.shape[0], X_test_full_scaled.shape[0]],
        'Accuracy': [metrics_selected['accuracy'], metrics_core['accuracy'], metrics_full_core['accuracy']],
        'Compression Ratio': [1, round(reduction_factor,2), 1],
        'Data Size (KB)': [X_train_full_scaled.nbytes / 1024, X_core.nbytes / 1024, X_test_full_scaled.nbytes / 1024],
