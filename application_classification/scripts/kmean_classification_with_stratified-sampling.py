...
def main():
    ...
    try:
        ...
        benchmark = Benchmarking(
            X_full=X_train_full_var, y_full=y_train_full,
            X_core=X_core, y_core=y_core,
            X_compressed=X_core_sampled, y_compressed=y_core_sampled,
            X_test=X_test_full_var, y_test=y_test_full,
            feature_names=selected_variance_feature_names,
            class_names=class_names
        )

        # Compare model performance across full, core, and compressed datasets
        benchmark.compare_model_performance(rf_full, rf_core, rf_core_sampled)

        # Perform statistical similarity tests between full and core datasets
        benchmark.statistical_similarity_tests()

        # Compare feature importance between full and core models
        benchmark.feature_importance_comparison(rf_full, rf_core)

        # Visualize feature distributions between full and core datasets
        benchmark.visualize_feature_distributions()

        # Evaluate clustering on the core dataset
        benchmark.clustering_evaluation()

        # Perform cross-validation checks on the core dataset
